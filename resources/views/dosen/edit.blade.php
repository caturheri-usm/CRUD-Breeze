    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="py-4 px-4 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ url('/update-dosen/'.$dosen->nidn) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div>
                        <x-input-label for="nidn" :value="__('NIDN')" />
                        <x-text-input id="nidn" class="block mt-1 w-full" type="text" name="nidn" :value="$dosen->nidn" required autofocus />
                        <x-input-error :messages="$errors->get('nidn')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="nama" :value="__('Nama')" class="mt-3"/>
                        <x-text-input id="nama" class="block mt-1 w-full" type="text" name="nama" :value="$dosen->nama" required autofocus />
                        <x-input-error :messages="$errors->get('nama')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="pendidikan" :value="__('Pendidikan')" class="mt-3"/>
                        <x-text-input id="pendidikan" class="block mt-1 w-full" type="text" name="pendidikan" :value="$dosen->pendidikan" required autofocus />
                        <x-input-error :messages="$errors->get('pendidikan')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="univ" :value="__('Universitas')" class="mt-3"/>
                        <x-text-input id="univ" class="block mt-1 w-full" type="text" name="univ" :value="$dosen->univ" required autofocus />
                        <x-input-error :messages="$errors->get('univ')" class="mt-2" />
                    <div>
                        <x-input-label for="foto" :value="__('Foto')" class="mt-3"/>
                        <x-text-input id="foto" class="block mt-1 w-full" type="file" accept="image/*" onchange="document.getElementById('output').src = window.URL.createObjectURL(this.files[0])" name="foto" :value="old('foto')" autofocus />
                        <x-input-error :messages="$errors->get('foto')" class="mt-2" />
                    </div>
                    <img src="{{ asset('storage/public/dosen/'.$dosen->foto) }}" class="mt-3" id="output" width="150px">
