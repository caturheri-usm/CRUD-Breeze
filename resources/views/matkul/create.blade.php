    ...
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="py-4 px-4 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ url('tambah-matkul') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div>
                    ...
                    <div>
                        ...
                        <x-text-input id="matkul" class="block mt-1 w-full" type="text" name="matkul" :value="old('matkul')" required autofocus />
                        <x-input-error :messages="$errors->get('matkul')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="dosen" :value="__('Dosen Pengajar')" class="mt-3" />
                        <x-text-input id="dosen" class="block mt-1 w-full" type="text" name="dosen" :value="old('dosen')" required autofocus />
                        <x-input-error :messages="$errors->get('dosen')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="sks" :value="__('SKS')" class="mt-3"/>
                        <x-text-input id="sks" class="block mt-1 w-full" type="number" name="sks" :value="old('sks')" required autofocus />
                        <x-input-error :messages="$errors->get('sks')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="total_mhs" :value="__('Total Mahasiswa')" class="mt-3"/>
                        <x-text-input id="total_mhs" class="block mt-1 w-full" type="number" name="total_mhs" :value="old('total_mhs')" required autofocus />
                        <x-input-error :messages="$errors->get('total_mhs')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="ruangan" :value="__('Ruangan')" class="mt-3"/>
                        <x-text-input id="ruangan" class="block mt-1 w-full" type="text" name="ruangan" :value="old('ruangan')" required autofocus />
